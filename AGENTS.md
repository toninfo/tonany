# tonany Development Notes

This repository is an independent customization of a Pi source snapshot under `toninfo/tonany`.
Do not reintroduce an upstream git remote, `@earendil-works/pi-*` package scope, or default runtime calls to `pi.dev`.
See `VENDOR_SNAPSHOT.md`.
