...
```bash
npm install @tonany/pi-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@tonany/pi-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@tonany/pi-telemetry';
...
`@tonany/pi-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@tonany/pi-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@tonany/pi-agent-core';
...
- `@tonany/pi-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@tonany/pi-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@tonany/pi-agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
