...
```ts
import { PiClient, type ByteTransportFactory } from "@tonany/pi-client";
...
```ts
import { PiClient } from "@tonany/pi-client";
import { createUnixTransportFactory } from "@tonany/pi-client/unix";
...
The `@tonany/pi-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@tonany/pi-client/unix` subpath.
