...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@tonany/pi-evals:session" && artifact.type !== "@tonany/pi-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@tonany/pi-evals:session" ? "sessions" : "sources";
