# @tonany/pi-session-backend-sqlite-node

Node `node:sqlite` Session backend for `@tonany/pi-agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@tonany/pi-agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@tonany/pi-session-backend-sqlite-node";
