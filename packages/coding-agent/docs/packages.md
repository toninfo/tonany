...
By default, `install` and `remove` write to user settings (`~/.tonany/agent/settings.json`). Use `-l` to write to project settings (`.tonany/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.tonany/agent/npm/`.
- Project installs go under `.tonany/npm/`.
...
- Cloned to `~/.tonany/agent/git/<host>/<path>` (global) or `.tonany/git/<host>/<path>` (project).
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@tonany/pi-ai`, `@tonany/pi-agent-core`, `@tonany/pi-coding-agent`, `@tonany/pi-tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. `pi config` starts in global settings (`~/.tonany/agent/settings.json`); press Tab to switch between global and project-local modes. Use `pi config -l` to start in project overrides (`.tonany/settings.json`) with inherited global resources dimmed.
