...
```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.tonany/agent/extensions/gondolin
cd ~/.tonany/agent/extensions/gondolin
...
```bash
cd /path/to/project
pi -e ~/.tonany/agent/extensions/gondolin
```
...
RUN npm install -g --ignore-scripts @tonany/pi-coding-agent
...
docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v pi-agent-home:/root/.tonany/agent \
  pi-sandbox
```

The `-v "$PWD:/workspace"` mounts your current directory into the container at /workspace such that reads and writes in `/workspace` inside Docker directly affect your host files, like in the Gondolin example.

Use a named volume for `/root/.tonany/agent` if you want container-local settings and sessions. Mounting your host `~/.tonany/agent` exposes host auth and session files to the container.
