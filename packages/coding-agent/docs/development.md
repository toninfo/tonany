...
See [AGENTS.md](https://github.com/toninfo/tonany/blob/main/AGENTS.md) for additional guidelines.

## Setup

```bash
git clone https://github.com/toninfo/tonany
cd pi
npm install
npm run build
```
...
`PI_SERVER_DIR` overrides the server profile and socket directory (default: `~/.tonany/server`). `PI_SERVER_ID` selects the logical server ID when `--server-id` is omitted.
...
`/debug` (hidden) writes to `~/.tonany/agent/pi-debug.log`:
