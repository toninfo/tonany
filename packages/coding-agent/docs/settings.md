...
| `.tonany/settings.json` | Project (current directory) |
...
Paths in `~/.tonany/agent/settings.json` resolve relative to `~/.tonany/agent`. Paths in `.tonany/settings.json` resolve relative to `.tonany`. Absolute paths and `~` are supported.
...
Project settings (`.tonany/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.tonany/agent/settings.json (global)
{
  "theme": "dark",
