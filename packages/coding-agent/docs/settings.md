...
Paths in `~/.tonany/agent/settings.json` resolve relative to `~/.pi/agent`. Paths in `.tonany/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
