...
Discovery rules:
- In `~/.tonany/agent/skills/` and `.tonany/skills/`, direct root `.md` files are discovered as individual skills when they have valid skill frontmatter with a non-empty `description`
...
For project-level Claude Code skills, add to `.tonany/settings.json`:
