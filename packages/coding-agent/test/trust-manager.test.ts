...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".tonany", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".tonany", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".tonany", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".tonany"), { recursive: true });
			writeFileSync(join(cwd, ".tonany", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".tonany"), { recursive: true, force: true });
