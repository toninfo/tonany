...
import { fauxAssistantMessage, fauxToolCall } from "@tonany/pi-ai";
import { describe, expect, it, vi } from "vitest";
import subagentExtension from "../../../examples/extensions/subagent/index.ts";
import type { ExtensionUIContext } from "../../../src/core/extensions/index.ts";
import { createHarness, getMessageText } from "../harness.ts";

vi.mock("@tonany/pi-coding-agent", () => ({
	CONFIG_DIR_NAME: ".tonany",
...
async function runProjectAgent(options: RunOptions): Promise<{ confirmCalls: number; toolResult: string }> {
	...
	try {
		const agentsDir = join(harness.tempDir, ".tonany", "agents");
