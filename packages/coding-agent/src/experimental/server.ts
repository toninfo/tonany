...
import { isServerId, type ServerId } from "@tonany/pi-protocol";
import {
...
} from "@tonany/pi-server";
import { createUnixServer, getUnixSocketPath } from "@tonany/pi-server/unix";
...
export function resolveServerDirectory(directory?: string): string {
	return resolvePath(directory ?? process.env[ENV_SERVER_DIR] ?? join(homedir(), ".tonany", "server"));
}
...
export interface StartServerOptions {
	/** Server profile and socket directory. Defaults to PI_SERVER_DIR or ~/.tonany/server. */
	readonly directory?: string;
