...
import { Markdown, type MarkdownTheme } from "@tonany/pi-tui";
...
const DEFAULT_INSTALLER_API_BASE = ""; // tonany: off unless PI_INSTALLER_API_BASE is set
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		...
		// tonany：无自建 installer 端点时拒绝托管自更新，避免回连上游
		if (!installerApiBase) {
			throw new Error(
				"Managed self-update requires PI_INSTALLER_API_BASE (TonAny does not use upstream pi.dev installers).",
			);
		}
