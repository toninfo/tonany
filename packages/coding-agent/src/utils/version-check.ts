...
// tonany 独立分支：默认不做上游版本探测；可通过 PI_LATEST_VERSION_URL 自建端点
const DEFAULT_VERSION_CHECK_TIMEOUT_MS = 10000;

function getLatestVersionUrl(): string {
	return process.env.PI_LATEST_VERSION_URL?.trim() ?? "";
}
...
export async function getLatestPiRelease(
	...
	// 未配置自建版本接口时直接跳过，避免回连上游 pi.dev
	if (!latestVersionUrl || process.env.PI_OFFLINE) return undefined;
