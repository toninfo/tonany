		...
		const manifest = {
			...
			...(isAgent ? {
				...
				dependencies: {
					...
					...(declareServer ? { "@tonany/pi-server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@tonany/pi-server";' : ""}
...
test("installs only coding-agent directly and uses overrides only for declared runtime dependencies", (t) => {
	...
	const nested = join(directory, "node_modules", codingAgentName, "node_modules/@tonany/pi-server");
	mkdirSync(nested, { recursive: true });
	writeFileSync(join(nested, "package.json"), JSON.stringify({ name: "@tonany/pi-server", version: "1.0.0" }));
